    ...
    function renderTable(data) {
      ...
      displayData.forEach((doc, i) => {
        ...
        row.innerHTML = `
          ...
          <td>${doc.tanggal}</td>
          <td><a href="${doc.file}" class="unduh-ic" download><img class="icon-img" src="${themeData.templateUrl}/assets/images/unduh merah.png" alt="Unduh"></a></td>
        `;
    ...
    function applyFilters() {
      ...
      const katEl = document.getElementById('filter-kategori');
      const tahunEl = document.getElementById('filter-tahun');
      
      if(!searchEl || !katEl || !tahunEl) return;
    ...
    function toggleLihatSemua() {
      ...
      if (!showAll) {
        ...
        if(docCard) docCard.scrollIntoView({ behavior: 'smooth' });
      }
    }

    function filterByCategory(kategori) {
      const katEl = document.getElementById('filter-kategori');
      if(katEl) katEl.value = kategori;
      showAll = true;
      applyFilters();
      const docCard = document.getElementById('docCard');
      if(docCard) docCard.scrollIntoView({ behavior: 'smooth' });
